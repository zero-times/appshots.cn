...
router.get('/:id', async (req, res, next) => {
  try {
    const sessionId = getSessionId(req);
    const [row] = await db.select().from(schema.projects).where(projectScope(req.params.id, sessionId, getUserId(req)));
    if (!row) {
...
router.patch('/:id', async (req, res, next) => {
  try {
    const sessionId = getSessionId(req);
    const scope = projectScope(req.params.id, sessionId, getUserId(req));
