...
## 5.2 项目状态
`draft -> analyzing -> ready -> exporting -> completed`
- 分析失败会回退到 `draft`
...
- 导出完成后：项目进入 `completed` 封存状态，仅保留 ZIP 下载地址；上传截图、AI 分析结果与生成文案会被清理
...
- 导出尺寸支持精简后的 App Store 与 Google Play 尺寸（iPhone 6.5"、iPad 13"、Android 手机与平板）
...
### 6.7 导出封存规则
- 项目导出成功后即封存（`status=completed`）
- 封存后不可再编辑文案/模板、不可再调整导出尺寸/语言、不可重复触发导出
- 前端应展示可点击下载地址，由用户主动触发 ZIP 下载
- 仅保留导出 ZIP 文件；上传图片与分析/文案数据会被服务端清理
